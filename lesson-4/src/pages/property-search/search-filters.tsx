...
export function PropertySearchFilters({
	...
	const handleAiSearchSubmit = (e: React.FormEvent) => {
		...
		startAITransition(async () => {
			...
			const searchText = formData.get("aiSearch");
			if (searchText && typeof searchText === "string") {
				const { error, filters: divinedFilters } = await aiSearch(searchText);
				if (error) {
					setAIError(error);
				} else if (divinedFilters) {
					setFilters(serializePropertySearchParams(divinedFilters));
				}
			}
		});
	};
			...
			<form onSubmit={handleAiSearchSubmit}>
				<div className="space-y-6 pb-6">
					<Fieldset>
						<Field>
							<Label>
								<span className="sr-only inline-flex items-center gap-2">
									AI Search
								</span>
							</Label>
							<Textarea
						...
						<div className="flex gap-3 pt-2">
							...
							>
								...
								{isAIPending ? "Divining..." : "AI search"}
							</Button>
						</div>
					</Fieldset>
				</div>
